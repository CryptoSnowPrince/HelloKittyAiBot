...
async def pending(message, username, origin_username):
    ...
    wait_t = 40 - wait_t
    if wait_t > 0:
        countdown_m = await message.reply('@' + origin_username + ' You\'re in countdown mode please wait ' + str(wait_t) + ' seconds before using Wonka again.')


@dp.message_handler(lambda message: (message.text not in config.commands and message.text.startswith("/wonka ")))
async def handle_input(message: aiogram.types.Message):
    prompt = message.text.split("/wonka ")[1]
    if prompt.strip() == "":
        return
...
def water_mark(image_url, username):
    ...
    # text = "I love WonkaTokenBot"

    # font = ImageFont.truetype('Anydore.otf', 100)
    ...
    # # draw watermark in the bottom right cosrner
    # draw.text((x, y), text, fill = (255,255,255,0), font=font, stroke_width=1, stroke_fill=(0,255,0))
